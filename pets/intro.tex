...
perform surveillance (notably, the United States~\cite{russia_secure_internet,routing_errors,dte}), this is a challenging problem with no known, effective 
...
introduces minimal performance overhead.  To summarize, we make the following key 
contributions:
...
States.
\item An analysis of how overlay network relays can help citizens, governments, or ISPs
discover and use paths that avoid certain countries.
\item An implementation and evaluation of a measurement-driven system, \system{}, that allows Internet users 
to easily avoid a given country.
\end{itemize}
