...
Informed by our analysis in Section \ref{datasets}, we now explore the extent
to which a system that explores and discovers alternate overlay network paths
...
global open DNS resolvers.  Due to space limitations, we omit our analysis of
how open DNS resolvers can improve diversity; we have included those results
in a technical report~\cite{characterizing_detours}. In this section, we
discuss our measurement method, develop an avoidance metric and algorithm, and
present our results for the use of overlay nodes to avoid specific countries.
...
Avoidability is the fraction of paths that originate in Country
...
\paragraph{Avoidability algorithm.}  Measuring the avoidability of Country Y
...
case value for avoidance.  The algorithm analyzes the destinations of all
...
%
Table \ref{tab:avoid} shows two significant trends: (1)~the ability
...
are known to perform surveillance on traffic are also often the most difficult countries
to avoid.
