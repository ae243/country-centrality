\section{\system{}: Routing Arond Nation-States}
\label{system_design}
...
that allow clients to access web content without installing custom software.
%in contrast, IP layer relays would require clients to install
%software, violating our usability goal.  
\system{} uses the
measurement methods described in Section~\ref{avoid_results} to learn paths between
clients, relays, and domains; these results are stored at the
oracle, which uses the data to decide which relay a client in
...
the system in more detail. %A client can then query the oracle to %determine
the appropriate relay to use to avoid a certain country %en route to a
