...
a given country.  We employ the country avoidance technique studied in Section 
\ref{avoid_results} --- an overlay network.  The overlay network 
...
As mentioned earlier, \system{} is a measurement-driven overlay network.  
...
Once the TCP proxies, or relays, are established, the system needs a way to 
...
hour.  Less than five paths from our results changed every two hours, and the 
results were similar for one hour increments as well.  As it takes approximately 30 minutes to 
...
As seen in Figure \ref{fig:paths}, there are four path components 
when a client accesses web content through \system{}: forward from the clients
to the relays, forward from the relays to domains, reverse from the domains 
to the relays, and reverse from the relays to the clients.  The system measures three 
...
location.  This decision follows four sequential steps:
...
\system{} is designed in a modular way, and therefore, adding relays is quick, easy, and 
simple. Additionally, \system{} is resilient to crashed system components, such as a 
crashed relay or oracle.  

{\bf Adding a Relay.} To add a relay, the system operator must set up a machine as a proxy 
...
{\bf Adding an Oracle.} Adding an oracle is a matter of installing the oracle software on 
