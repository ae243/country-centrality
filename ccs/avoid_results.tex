...
countries would never reach 1.0.  For this reason, we measured the upper
bound on avoidance for a given pair of (Country X, Country Y) that
represents the best case value for avoidance.  The pseudocode for the
algorithm is shown in Algorithm \ref{upperbound_algo}. 
...
of paths when using open resolvers, whereas they can only avoid 40\% of
paths when using relays.  The difference in avoidance values can be
...
(because all paths from the client to the other relays traverses the
...
The avoidance value for Kenyan clients avoiding South Africa is 0.66,
...
clients can get a significantly better avoidance value (0.87) when using
