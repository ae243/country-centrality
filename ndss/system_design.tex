...
Some of these obstacles can be completely solved if content providers contributed to the surveillance 
...
replicate content in specific regions in order to allow clients to access replicas without traversing a specific 
country. 

While we do not have access to any content providers, we take an approach at designing 
a system that is a first countermeasure to large-scale surveillance {\it without} the help 
of providers.  Due to this choice, it is not a perfect system, and it suffers from the obstacles faced 
during the measurement studies and highlighted in this section.  This first system design, called \system{}, shows 
the extent to which we can avoid an unfavorable country without the participation of 
content providers, and it invites future work and systems that also address the problem 
