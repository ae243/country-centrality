...
for line in f:
    domain_weights[line.strip()] = float(inc/100.0)
    inc += 1
f.close()
...
for d in domain_endpoint:
    ...
    denom += float(domain_weights[d])
    if len(ends) == 1:
        co = pycountry.countries.get(alpha2=y)
        if ends[0] == co.name:
            upper += float(domain_weights[d])


upper = 1.0 - (upper/denom)
...
total_avoidable = 0.0
for d in domain_map:
    total_avoidable += float(domain_weights[d])

total = 0.0
for d in domain_exist:
    total += float(domain_weights[d])
